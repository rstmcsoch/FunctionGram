...
export async function seed(){
  ...
  content.forEach((c,i)=>{statements.push(database.prepare('INSERT OR IGNORE INTO posts (id,author_id,media,media_type,kind,caption,location,category,base_likes,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)').bind('demo_'+c[0],'demo_'+c[1],JSON.stringify(['/media/'+c[0]+'.jpg']),'image','post',c[2],c[3],c[4],c[5],now-(i+1)*7200000));
    if(i<7)statements.push(database.prepare('INSERT OR IGNORE INTO posts (id,author_id,media,media_type,kind,caption,location,category,base_likes,created_at,expires_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)').bind('story_'+c[0],'demo_'+c[1],JSON.stringify(['/media/'+c[0]+'.jpg']),'image','story',c[3],c[3],c[4],0,now-(i+1)*1200000,now+86400000));
  });
  ...
  statements.push(database.prepare('INSERT OR IGNORE INTO posts (id,author_id,media,media_type,kind,caption,location,category,base_likes,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)').bind('demo_flowers','demo_emily',JSON.stringify(['/media/flowers.mp4']),'video','reel','A few seconds of stillness. 🌸 #nature #littlethings','In the garden','Nature',318,now-80000000));
