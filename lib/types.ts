export type Person={id:string;username:string;name:string;bio:string;website?:string;avatar:string;is_demo:number;followers:number;following:number;post_count:number;followed:number};
export type MediaOption={ratio:'original'|'1:1'|'4:5'|'16:9';fit:'contain'|'cover';alt:string};
export type Post={id:string;author_id:string;media:string[];aspects?:number[]|null;media_options?:MediaOption[];tagged_users?:string[];highlighted?:boolean;media_type:'image'|'video';kind:'post'|'reel'|'story';caption:string;location:string;category:string;created_at:number;expires_at:number|null;likes:number;liked:number;saved:number;seen:number;comment_count:number;comment_preview?:{body:string;username:string}|null;author:Person};
...
export type Notification={id:string;actor_id:string;kind:string;post_id:string|null;created_at:number;read_at:number|null;username:string;avatar:string;media:string|null;media_type:string|null};
