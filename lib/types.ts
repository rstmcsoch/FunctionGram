...
export type Post={id:string;author_id:string;media:string[];aspects:number[]|null;media_type:'image'|'video';kind:'post'|'reel'|'story';caption:string;location:string;category:string;created_at:number;expires_at:number|null;likes:number;liked:number;saved:number;seen:number;comment_count:number;author:Person};
