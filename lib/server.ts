...
export async function feed(viewer:string|null,limit=40,offset=0,filter:{author?:string;post?:string;saved?:boolean}={}):Promise<Post[]>{
  ...
  return r.results.map(p=>({...p,media:JSON.parse(p.media as string),author:{id:p.author_id,username:p.username,name:p.name,avatar:p.avatar,bio:p.bio,is_demo:p.is_demo}})) as unknown as Post[];
}
